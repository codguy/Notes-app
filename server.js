...
app.get('/', (req, res) => {
  ...
  // renderView(res, 'Home', {block:"index", data:{title:"Satnam", notes: notes}});
});
...
app.post('/create', (req, res, next) => {
    ...
    fs.writeFile('./data/' + req.body.id + '.json', JSON.stringify(req.body), 'utf8', function (err) {
        if (err) {
...
app.post('/archive/:id', (req, res) => {
  fs.rename('./data/' + req.params.id + '.json', './archived/' + req.params.id + '.json', function (err) {
    if (err) {
      console.error(err.message);
...
app.post('/unarchive/:id', (req, res) => {
  fs.rename('./archived/' + req.params.id + '.json', './data/' + req.params.id + '.json', function (err) {
    if (err) {
      console.error(err.message);
      res.status(500).json({ success: false, message: err.message });
    } else {
